...
from app.utils.data_loader import campaign_goals_df, summary_df,segment_df
...
def build_insight_context(
    ...
    if campaign_id is None:
        ...
        #     campaign_id=None,
        # )
        # signal_context = format_segment_signals(signals) #if signal_context == format_segment_signals(signals) else signal_context
    ...
    #     raise ValueError(
    #         f"No campaign-level data was found for {campaign_id}."
    #     )
    ...
    for _, row in campaign_summary_rows[campaign_columns].iterrows():
        context_lines.extend(
            [
                f"Segment: {row['customer_segment']}",
                f"- Audience: {row['audience_size']:,}",
                f"- Revenue: ${row['campaign_revenue']:,.2f}",
...
# print(build_insight_context(
#     summary_df=summary_df,
#     segment_df=segment_df,
#     campaign_goals_df=campaign_goals_df,
#     campaign_id=None,
#     signal_context=format_segment_signals(signals)),
# )